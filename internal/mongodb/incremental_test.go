...
func TestIncrementalCopy(t *testing.T) {
	...
	destClient.GetDatabase(dbName).Collection(collName).Drop(ctx)
	...
	err = CopyCollection(ctx, sourceDB, destDB, collName, true, 10)
	require.NoError(t, err, "Failed to copy collection")

	// Verify documents were copied
	destColl := destClient.GetDatabase(dbName).Collection(collName)
	count, err := destColl.CountDocuments(ctx, bson.M{})
